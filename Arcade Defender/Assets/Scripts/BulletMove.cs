...
public class BulletMove : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == "Lander")
        {
            ...
            lander?.Kill();
            HandelAfterCollision();
        }
        if (collision.tag == "Baiter")
        {
            var baiter = collision.GetComponent<MoveBaiter>();
            baiter?.Kill();
            HandelAfterCollision();
        }
    }

    private void HandelAfterCollision()
    {
        _onKill?.Invoke();//update palayer score
        Destroy(gameObject);
    }
}
